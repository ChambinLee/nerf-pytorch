...
def config_parser():
    ...
    parser.add_argument("--N_rand", type=int, default=32 * 32 * 4,  # why set num like this?
                        help='batch size (number of random rays per gradient step)')
    parser.add_argument("--lrate", type=float, default=5e-4,
                        help='learning rate')
    parser.add_argument("--lrate_decay", type=int, default=250,  # what's the formula of this?
...
def train():
    ...
    if args.dataset_type == 'llff':
        ...
        hwf = poses[0, :3, -1]
        # shape: poses [20,3,4] hwf给出去之后把每一行的第5个元素(最后一个)删掉
        poses = poses[:, :3, :4]
    ...
    for i in trange(start, N_iters):
        ...
        if use_batching:
            ...
            batch = rays_rgb[i_batch:i_batch + N_rand]  # [B, 2+1, 3*?],取一个batch, batch shape：[4096,3,3]
            batch = torch.transpose(batch, 0, 1)  # 转换0维和1维的位置[ro+rd+rgb,4096,3]
            batch_rays, target_s = batch[:2], batch[2]  # shape: batch_rays shape[ro+rd,4096,3] target_s[4096,3]对应的是rgb

            i_batch += N_rand

            if i_batch >= rays_rgb.shape[0]:  # 如果所有样本都遍历过了则打乱数据
        ...
        # 在extra里面的一个元素，求损失并加到整体损失上 (what's this?)
        if 'rgb0' in extras:
